...
# ------------------------------------------------------------------------
# Modified from Torch (https://github.com/pytorch/pytorch)
...
from .mha import HydraMultiheadAttention
from ..layers import HydraLinear, HydraLayerNorm
from .transformer import _get_activation_fn, _slice_embedding_dim

class HydraTransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.self_attn = HydraMultiheadAttention(d_model, nhead, dropout=dropout)
        # Implementation of Feedforward model
        self.linear1 = HydraLinear(d_model, dim_feedforward)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = HydraLinear(dim_feedforward, d_model)
    ...
    def forward_post(self,
        ...
        out = self.linear2(self.dropout(self.activation(self.linear1(src, in_feature=effective_dim))), out_feature=effective_dim)
    ...
    def forward_pre(self, src,
        ...
        out = self.linear2(self.dropout(self.activation(self.linear1(out, in_feature=effective_dim))), out_feature=effective_dim)
        src = _slice_embedding_dim(src, effective_embed_dim=effective_dim) + self.dropout2(out)
        return src
...
class HydraTransformerEncoder(nn.Module):
...
class HydraTransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.head_dim = d_model // nhead
        self.self_attn = HydraMultiheadAttention(d_model, nhead, dropout=dropout)
        self.multihead_attn = HydraMultiheadAttention(d_model, nhead, dropout=dropout)
        # Implementation of Feedforward model
        self.linear1 = HydraLinear(d_model, dim_feedforward)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = HydraLinear(dim_feedforward, d_model)
        ...
        self.norm1 = HydraLayerNorm(d_model)
        self.norm2 = HydraLayerNorm(d_model)
        self.norm3 = HydraLayerNorm(d_model)
        self.dropout1 = nn.Dropout(dropout)
        self.dropout2 = nn.Dropout(dropout)
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt, in_feature=effective_embed_dim))), out_feature=effective_embed_dim)
    ...
    def forward_pre(self, tgt, memory,
        ...
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt2, in_feature=effective_embed_dim))), out_feature=effective_embed_dim)
        tgt = _slice_embedding_dim(tgt, effective_embed_dim=effective_embed_dim) + self.dropout3(tgt2)
        return tgt
...
class HydraTransformerDecoder(nn.Module):
...
class HydraTransformer(nn.Module):
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        encoder_layer = HydraTransformerEncoderLayer(d_model, nhead, dim_feedforward,
                                                dropout, activation, normalize_before)
        encoder_norm = HydraLayerNorm(d_model) if normalize_before else None
        self.encoder = HydraTransformerEncoder(encoder_layer, num_encoder_layers, encoder_norm)

        decoder_layer = HydraTransformerDecoderLayer(d_model, nhead, dim_feedforward,
                                                dropout, activation, normalize_before)
        decoder_norm = HydraLayerNorm(d_model)
        self.decoder = HydraTransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
...
def build_transformer(args):
    print("Building Hydra Transformer")
