...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for idx, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if heads is not None:
            # Single forward pass, compute losses for ALL head configs
            all_outputs = model.forward_multi_head(samples, heads)

            total_losses = 0
            combined_loss_dict = {}
            for eff_k, outputs in all_outputs.items():
                loss_dict_k = criterion(outputs, targets)
                losses_k = sum(loss_dict_k[key] * weight_dict[key]
                               for key in loss_dict_k.keys() if key in weight_dict)
                total_losses += losses_k
                for key, val in loss_dict_k.items():
                    if key not in combined_loss_dict:
                        combined_loss_dict[key] = val
                    else:
                        combined_loss_dict[key] = combined_loss_dict[key] + val

            # Average over head configs
            n_heads = len(heads)
            losses = total_losses / n_heads
            loss_dict = {key: val / n_heads for key, val in combined_loss_dict.items()}
        else:
        ...
        if idx % print_freq == 0 or idx == len(data_loader) - 1:
            if heads is not None:
                print("Multi-head training:", heads)
            else:
