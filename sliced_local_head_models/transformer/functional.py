...
def _head_local_input_projection(
    q: Tensor,
    k: Tensor,
    v: Tensor,
    w_q: Tensor,
    ...
    """Block-diagonal in-projection: each head projected independently.

    w_q, w_k, w_v: [nhead, head_dim, head_dim] (out_dim first), b_q, b_k, b_v: [nhead, head_dim] or None.
    q, k, v: [..., nhead * head_dim].
    """
    eh = effective_heads if effective_heads is not None else num_heads
    q_h = q[..., :eh * head_dim].unflatten(-1, (eh, head_dim))
    k_h = k[..., :eh * head_dim].unflatten(-1, (eh, head_dim))
    v_h = v[..., :eh * head_dim].unflatten(-1, (eh, head_dim))

    # [..., eh, 1, head_dim] @ [eh, head_dim, head_dim] → [..., eh, 1, head_dim] → [..., eh, head_dim]
    q_proj = (q_h.unsqueeze(-2) @ w_q[:eh].permute(0, 2, 1)).squeeze(-2)
    k_proj = (k_h.unsqueeze(-2) @ w_k[:eh].permute(0, 2, 1)).squeeze(-2)
    v_proj = (v_h.unsqueeze(-2) @ w_v[:eh].permute(0, 2, 1)).squeeze(-2)

    if b_q is not None:
        q_proj = q_proj + b_q[:eh]
    if b_k is not None:
        k_proj = k_proj + b_k[:eh]
    if b_v is not None:
        v_proj = v_proj + b_v[:eh]

    return q_proj.flatten(-2), k_proj.flatten(-2), v_proj.flatten(-2)


def _head_local_output_projection(attn_output, out_w, out_b, effective_heads, head_dim):
    """Block-diagonal output projection: each head projected independently.
    
    out_w: [nhead, head_dim, head_dim], out_b: [nhead, head_dim] or None
    attn_output: [n, k * head_dim] where k = effective_heads
    """
    k = effective_heads
    x = attn_output.unflatten(-1, (k, head_dim))        # [n, k, head_dim]
    # Per-head projection: out[n,k,d] = sum_e x[n,k,e] * w[k,d,e]
    # x[n,k,1,e] @ out_w[k,e,d] -> [n,k,1,d] -> [n,k,d]
    out = (x.unsqueeze(-2) @ out_w[:k].permute(0, 2, 1)).squeeze(-2)
    if out_b is not None:
        out = out + out_b[:k]
    return out.flatten(-2)                                # [n, k * head_dim]
...
def multi_head_attention_forward(
    ...
    if not use_separate_proj_weight:
        ...
        if in_proj_weight.ndim == 4:
            # head-local packed weights: [3, nhead, head_dim, head_dim], bias [3, nhead, head_dim] or None
            _b_q = in_proj_bias[0] if in_proj_bias is not None else None
            _b_k = in_proj_bias[1] if in_proj_bias is not None else None
            _b_v = in_proj_bias[2] if in_proj_bias is not None else None
            q, k, v = _head_local_input_projection(
                query, key, value,
                in_proj_weight[0], in_proj_weight[1], in_proj_weight[2],
                _b_q, _b_k, _b_v,
                effective_heads=effective_heads, num_heads=num_heads, head_dim=head_dim,
            )
        elif effective_heads is not None and effective_heads < num_heads:
        ...
        if q_proj_weight.ndim == 3:
            # head-local separate weights: [nhead, head_dim, head_dim], bias [nhead*head_dim] reshaped
            b_q_h = b_q.view(num_heads, head_dim) if b_q is not None else None
            b_k_h = b_k.view(num_heads, head_dim) if b_k is not None else None
            b_v_h = b_v.view(num_heads, head_dim) if b_v is not None else None
            q, k, v = _head_local_input_projection(
                query, key, value,
    ...
    if need_weights:
        ...
        # Output projection — head-local (3D weights) or dense (2D weights)
        if out_proj_weight.ndim == 3:
            attn_output = _head_local_output_projection(attn_output, out_proj_weight, out_proj_bias, effective_heads, head_dim)
        elif effective_heads is not None and effective_heads < num_heads:
