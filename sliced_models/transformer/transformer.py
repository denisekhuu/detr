...
from ..layers.norm import SlicedLayerNorm
...
class SlicedTransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.linear1 = SlicedLinear(d_model, dim_feedforward)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = SlicedLinear(dim_feedforward, d_model)

        self.norm1 = SlicedLayerNorm(d_model)
        self.norm2 = SlicedLayerNorm(d_model)
...
class SlicedTransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.norm1 = SlicedLayerNorm(d_model)
        self.norm2 = SlicedLayerNorm(d_model)
        self.norm3 = SlicedLayerNorm(d_model)
        self.dropout1 = nn.Dropout(dropout)
        self.dropout2 = nn.Dropout(dropout)
...
class SlicedTransformer(nn.Module):
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        encoder_norm = SlicedLayerNorm(d_model) if normalize_before else None
        ...
        decoder_layer = SlicedTransformerDecoderLayer(d_model, nhead, dim_feedforward,
                                                dropout, activation, normalize_before)
        decoder_norm = SlicedLayerNorm(d_model)
